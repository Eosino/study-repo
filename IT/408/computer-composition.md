...
* 其他进制转十进制：将每一位与其权重相乘再累加，如：$(1010)_2=1\times2^3+1\times2^1=10$，$(F5)_{16}=15\times16^1+5\times16^0=245$
* 十进制转其他进制：
...
|-|-|-|-|-|-|
|原码|符号位0表示正数，1表示负数|真值转为二进制，补零，确定符号位|$-(2^n-1)~2^n-1$|[00...0]或[10...0]|表示IEEE754浮点数的尾数|
|反码|若符号位是1则将数值位逐位翻转|正数时，反码=原码；负数时，数值位翻转|$-(2^n-1)~2^n-1$|[00...0]或[10...0]|数码变换的中间形式|
|补码|用加法来实现减法运算|正数时，补码=反码=原码；负数时，反码末位+1|$-2^n~2^n-1$|[00...0]|表示计算机中的有符号数|
|移码|可以按无符号数比较的大小反映真值的相对大小|对补码的符号位取反得移码|$-2^n~2^n-1$|[10...0]|表示浮点数的指数|
