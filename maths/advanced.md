...
6. 导数介值定理（达布定理）：若$f(x)$在$[a,b]$上可导，且$f_+'(x)f_-'(b)<0$，则至少存在$\varepsilon\in(a,b)$使得$f'(\varepsilon)=0$
...
3. 介值定理：若 $f(a)=A,\space f(b)=B$，那么对于 $\forall C\in (A,B)$，$\exist c\in(a,b)$ 使 $f(c)=C$
4. 介值定理推论：若 $m\le C\le M$，则 $\exists c\in[a,b]$ 使 $f(c)=C$
5. 零点定理：若 $f(a)f(b)\lt 0$，则 $\exist c\in(a,b)$ 使 $f(c)=0$
6. 函数平均值定理：若 $a\lt x_1\lt x_2\lt ...\lt x_n\lt b$，则 $\exist \xi\in[x_1,x_n]$ 使 $f(\xi)=\frac{f(x_1)+f(x_2)+...+f(x_n)}{n}$

## 2.6. 中值定理
|  |  |  |  |
|---|---|---|---|
| 罗尔定理 | $f(x)$ 在 $[a,b]$ 上连续，在 $(a,b)$ 内可导，$f(a)=f(b)$ | $\exist \xi\in (a,b)$ | $f'(\xi)=0$ |
| 拉格朗日中值定理 | $f(x)$ 在 $[a,b]$ 上连续，在 $(a,b)$ 内可导 | $\exist \xi\in (a,b)$ | $f'(\xi)=\frac{f(b)-f(a)}{b-a}$ |
| 柯西中值定理 | $f(x),g(x)$ 在 $[a,b]$ 上连续，在 $(a,b)$ 内可导，且 $g'(x)\neq 0$ | $\exist \xi\in (a,b)$ | $\frac{f'(\xi)}{g'(\xi)}=\frac{f(b)-f(a)}{g(b)-g(a)}$ |
| 积分中值定理<br>（介值定理可证） | $F(x)=\int_a^xf(t)dt$，在 $[a,b]$ 上连续，在 $(a,b)$ 内可导 | $\exist \xi\in[a,b]$ | $\int_a^bf(x)dx=f(\xi)(b-a)$ |
| 积分中值定理<br>（升级版，拉格朗日中值定理可证） | $F(x)=\int_a^xf(t)dt$，在 $[a,b]$ 上连续，在 $(a,b)$ 内可导 | $\exist \xi\in(a,b)$ | $\int_a^bf(x)dx=f(\xi)(b-a)$ |
|  |  |  |  |
...
|---|---|---|---|
| 驻点 | 导数为0的点 |  |  |
| 极值 | 比附近点大（小） | 极值+可导 $\rArr f'(x_0)=0$ | 1. 左右导数异号；左正右负极大值，左负右正极小值<br>2. $f'(x_0)=0$ 且 $f''(x_0)\ne 0$ ；$f''(x_0)>0$ 极小值，$f''(x_0)<0$ 极大值<br>3. $1\sim n-1$ 阶导数都为0，$f^{(n)}(x_0)\ne 0$，且 $n$ 为偶数（仅作了解） |
| 拐点 | 凹凸分界点 | 拐点+二阶可导 $\rArr f''(x_0)=0$ | 1. 二阶导数异号<br>2. $f''(x_0)=0$ 且 $f'''(x_0)\ne 0$<br>3. $1\sim n-1$ 阶都为0，$f^{(n)}(x_0)\ne 0$，且 $n$ 为奇数（仅作了解） |
|  |  |  |  |
