...
5. 导数极限定理：若$f(x)$在点$x_0$的左邻域$[x_0-\delta,x_0]$上连续，在$(x_0-\delta,x_0)$内可导，且$\lim\limits_{x\to x^-_0}f'(x)$存在，则$\lim\limits_{x\to x^-_0}f'(x)=f'_-(x_0)$，即：函数$f(x)$在$x_0$的左导数等于导函数$f'(x)$在$x_0$的左极限
   * 同理：若$f(x)$在点$x_0$的右邻域$[x_0,x_0+\delta]$上连续，在$(x_0,x_0+\delta)$内可导，且$\lim\limits_{x\to x^+_0}f'(x)$存在，则$\lim\limits_{x\to x^+_0}f'(x)=f'_+(x_0)$，即：函数$f(x)$在$x_0$的右导数等于导函数$f'(x)$在$x_0$的右极限
   * 用导数定义及拉格朗日中值定理即可证明，由此可以证明导函数至多有第二类间断点，即具有第一类间断点的函数一定没有原函数
6. 导数介值定理（达布定理）：若$f(x)$在$[a,b]$上可导，且$f'_+(a)f'_-(b)<0$，则至少存在$\xi\in(a,b)$使得$f'(\xi)=0$
